...
const generateLedgerStyle = (state: AppState): string => {
  ...
  VIRTUES.forEach((virtue) => {
    ...
    records.forEach((record, index) => {
      const faults = record.observations.filter((observation) => observation.hasFault).length;
      ...
      const notesWithFaults = record.observations.filter((observation) => observation.hasFault && observation.note);
      if (notesWithFaults.length > 0) {
        content += '  Notes:\n';
        notesWithFaults.forEach((observation) => {
          const date = new Date(observation.date).toLocaleDateString('en-US', { month: 'short', day: 'numeric' });
          content += `    • ${date}: ${observation.note}\n`;
        });
      }
      content += '\n';
    });
  ...
  if (currentYear > startYear) {
    ...
    const yearRecords = state.weekRecords.filter((record) => new Date(record.startDate).getFullYear() === currentYear);
    content += `Weeks practiced this year: ${yearRecords.length}\n\n`;

    VIRTUES.forEach((virtue) => {
      const virtueRecords = yearRecords.filter((record) => record.virtueId === virtue.id);
      if (virtueRecords.length > 0) {
        const totalFaults = virtueRecords.reduce((sum, record) => sum + record.observations.filter((observation) => observation.hasFault).length, 0);
...
export const exportCharacterRecord = async (state: AppState): Promise<void> => {
  ...
  try {
    ...
    await Share.share({ title: 'Export Character Record', message: content, url: file.uri });
  } catch (error) {
